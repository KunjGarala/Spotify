...
async function getsong(folder) {
  ...
  const a = await fetch(`https://jade-zuccutto-ac1518.netlify.app/${folder}/`);
...
async function displayAlbums() {
  const a = await fetch(`https://jade-zuccutto-ac1518.netlify.app/song/`);
  const response = await a.text();
  const div = document.createElement("div");
  div.innerHTML = response;
  ...
  for (let index = 0; index < array.length; index++) {
    ...
    if (e.href.includes("song/")) {
      ...
      const a = await fetch(`https://jade-zuccutto-ac1518.netlify.app/song/${folder}/detail.json`);
